...
<?php
  ...
  $table = "XXXX";
  $sql = "select update_time from information_schema.tables where table_schema = 'oprate' and table_name = '$table'";
...
function disk() {
  ...
  $.ajax({
    ...
    url: "reload_disk.php",
    dataType: "json",
    data: {clus: <?php echo json_encode($clus); ?>},
  }).done(function(data){
    remove();
    input();
    target.value="更新"
...
function input(){
  for (i=0;i<data.length;i++) {
    ...
    dataset =[{name: "Used", value: data[i][1], "link": link},{name: "Avail", value:data[i][2], "link": link} ]
    render();
  }
}
